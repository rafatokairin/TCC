...
% ---
\begin{resumo}
 Esta pesquisa apresenta um estudo exploratório de viabilidade sobre o uso de dados sintéticos e técnicas de aprendizado de máquina para a ampliação de conjuntos de dados de mamografia, não constituindo evidência de robustez clínica nem ferramenta de apoio ao diagnóstico. A partir da base pública CBIS-DDSM (\textit{Curated Breast Imaging Subset of DDSM}), foi construído um protocolo experimental reprodutível e livre de vazamento de dados (\textit{data leakage}) para medir o efeito de mamografias sintéticas sobre um classificador. As imagens são separadas em nível de paciente em uma partição de desenvolvimento (DEV) e um conjunto de teste (TEST) bloqueado. Um modelo generativo \textit{StyleGAN2-ADA condicional} é treinado exclusivamente sobre DEV, e as imagens sintéticas são filtradas pela métrica \textit{Learned Perceptual Image Patch Similarity} (LPIPS) referenciando apenas as imagens reais de DEV; a fidelidade e a diversidade da geração são adicionalmente avaliadas pelas métricas FID e KID. O classificador \textit{EfficientNet-B0} é treinado com dados reais e sintéticos de DEV, com seleção de modelo em uma validação interna, e avaliado no TEST bloqueado, repetindo-se o procedimento por múltiplas sementes para viabilizar análise estatística. Diferentes proporções de imagens sintéticas são incorporadas ao treino (1:1, 2:1, 3:1 e 4:1), mantendo a avaliação restrita a imagens reais do TEST. Como verificação de generalização, o modelo treinado no CBIS-DDSM é adicionalmente avaliado, sem retreinamento, em uma segunda base (MIAS), ficando a validação multi-\textit{scanner} mais ampla como trabalho futuro. A contribuição central é um protocolo reprodutível e livre de vazamento para mensurar os efeitos do aumento com dados sintéticos, acompanhado de uma análise honesta de se, e em que condições, o aumento ajuda ou prejudica, com significância avaliada por testes estatísticos. Sob o protocolo livre de vazamento, com separação em nível de paciente, classificador funcional (ajustado por \textit{fine-tuning}) e fidelidade limitada do gerador, a augmentação com imagens sintéticas \textbf{não} melhorou a classificação: nenhuma proporção (1:1, 2:1, 3:1 ou 4:1) superou ou diferiu significativamente da linha de base composta apenas por dados reais, que foi a melhor configuração (teste de Wilcoxon pareado com correção de Holm, $p_{\text{aj}} = 1{,}000$ para AUC e acurácia; tamanhos de efeito nulos ou negativos, delta de Cliff entre $-0{,}18$ e $+0{,}12$; teste de DeLong não significativo, $p$ entre 0,124 e 0,513); não houve memorização (0\% das amostras sinalizadas). A fidelidade do gerador foi limitada (FID em torno de 200--250, ante 20--60 de GANs de mamografia de alta qualidade), o que é central para interpretar o resultado nulo. Uma análise de ablação mostra que o ganho aparentemente significativo reportado sob protocolos mais fracos (AUC de 0,611 para 0,683) só é recuperável quando se emprega separação em nível de imagem, que permite vazamento de dados de uma mesma paciente entre treino e teste, caracterizando esse ganho como artefato metodológico, e não como efeito real dos dados sintéticos. A validação externa no MIAS, sem retreinamento, confirmou a ausência de benefício (AUC da linha de base 0,533 e das proporções sintéticas entre 0,49 e 0,50, no nível do acaso e abaixo da linha de base, sem significância), reforçando o caráter exploratório e não clínico do estudo. O limiar $\tau = 0{,}2$ empregado na versão anterior mostrou-se inviável sob o protocolo corrigido, assim como todo $\tau \le 0{,}25$ (retenção de amostras benignas inferior a 1\%), tendo-se adotado $\tau = 0{,}30$. Não se afirma que a augmentação sintética não possa ajudar; mostra-se que, com um gerador de fidelidade moderada e avaliação rigorosa, os ganhos comumente reportados não se reproduzem. Esses números substituem integralmente os da versão anterior, obtidos sob arranjo com vazamento de dados.
\end{resumo}
% ---
...
\begin{Abstract}
 This work presents an exploratory feasibility study on the use of synthetic data and machine learning techniques to augment mammography datasets; it does not constitute evidence of clinical robustness nor a diagnostic-support tool. Using the public CBIS-DDSM (Curated Breast Imaging Subset of DDSM) database, we build a reproducible, leakage-free experimental protocol to measure the effect of synthetic mammograms on a downstream classifier. Images are split at the patient level into a development partition (DEV) and a locked test set (TEST). A conditional StyleGAN2-ADA generative model is trained only on DEV, and the synthetic images are filtered with the Learned Perceptual Image Patch Similarity (LPIPS) metric referencing only the DEV real images; generation fidelity and diversity are additionally assessed with the FID and KID metrics. The EfficientNet-B0 classifier is trained on DEV real and synthetic data, with model selection on an inner validation split, and evaluated on the locked TEST set, repeating the procedure over multiple seeds to enable statistical analysis. Different proportions of synthetic images are incorporated into training (1:1, 2:1, 3:1, and 4:1), while evaluation remains restricted to real TEST images. As a generalization check, the model trained on CBIS-DDSM is additionally evaluated, without retraining, on a second dataset (MIAS), leaving broader multi-scanner validation as future work. The main contribution is a reproducible, leakage-free protocol for measuring the effects of synthetic-data augmentation, together with an honest analysis of whether, and under what conditions, augmentation helps or hurts, backed by significance testing. Under the leakage-free protocol, with patient-level splitting, a functional (fine-tuned) classifier and limited generator fidelity, synthetic augmentation did \textbf{not} improve classification: no ratio (1:1, 2:1, 3:1, or 4:1) exceeded or significantly differed from the real-data-only baseline, which was the best configuration (paired Wilcoxon test with Holm correction, $p_{\text{adj}} = 1.000$ for AUC and accuracy; null or negative effect sizes, Cliff's delta between $-0.18$ and $+0.12$; DeLong test not significant, $p$ between 0.124 and 0.513); no memorization was observed (0\% of samples flagged). Generator fidelity was limited (FID around 200--250, versus 20--60 for high-quality mammography GANs), which is central to interpreting the null result. An ablation shows that the apparently significant gain reported under weaker protocols (AUC 0.611 to 0.683) is only recoverable when an image-level split is used, which allows same-patient leakage between training and test, characterizing that gain as a methodological artifact rather than a real effect of the synthetic data. External validation on MIAS, without retraining, confirmed the absence of benefit (baseline AUC 0.533 and synthetic-ratio AUCs between 0.49 and 0.50, at chance level and below the baseline, with no significance), reinforcing the exploratory, non-clinical framing of the study. The $\tau = 0.2$ threshold used in the previous version proved infeasible under the corrected protocol, as did any $\tau \le 0.25$ (retaining fewer than 1\% of benign samples), and $\tau = 0.30$ was adopted instead. We do not claim that synthetic augmentation cannot help; we show that, with a moderate-fidelity generator and rigorous evaluation, the commonly reported gains do not reproduce. These figures fully replace those of the previous version, which were obtained under a data-leakage setup.
\end{Abstract}
% ---
...
Cada imagem gerada foi comparada com as amostras reais de DEV, e o valor mínimo de LPIPS obtido foi utilizado como medida de similaridade. O limiar de seleção não foi fixado a priori: conforme descrito anteriormente, ele resulta de uma varredura de limiares ($\tau \in \{0{,}1;\ 0{,}15;\ 0{,}2;\ 0{,}25;\ 0{,}3\}$) que equilibra fidelidade e diversidade, adotando-se $\tau = 0{,}30$ como ponto de operação, uma vez que todo $\tau \le 0{,}25$ retinha menos de 1\% das amostras benignas sob o protocolo livre de vazamento, inviabilizando a construção do conjunto sintético. Uma verificação de vizinho mais próximo foi aplicada para sinalizar e contabilizar eventuais cópias quase idênticas às imagens reais, mitigando o risco de memorização.
...
A análise quantitativa da geração revelou uma fidelidade limitada, aspecto central para a interpretação do resultado nulo de classificação. A varredura de limiares mostrou que o valor $\tau = 0{,}2$, empregado no trabalho original, tornou-se inviável sob o protocolo livre de vazamento, assim como todo $\tau \le 0{,}25$, uma vez que resultaram em retenção inferior a 1\% das amostras da classe benigna; adotou-se, portanto, $\tau = 0{,}30$, que produziu aceitação de 7\% para a classe benigna e 21\% para a maligna. A verificação de memorização não sinalizou nenhuma amostra (0\%, $n_{\text{sinalizadas}} = 0$): o menor valor de LPIPS entre as imagens retidas e seus vizinhos reais mais próximos foi de 0,282 para a classe benigna e 0,277 para a maligna, valores muito acima do limiar de memorização de 0,05, descartando a hipótese de cópia das imagens de treino. Quanto à fidelidade e à diversidade agregadas, obteve-se FID de 203,5 (benigno) e 247,0 (maligno) e KID de $0{,}198 \pm 0{,}008$ (benigno) e $0{,}273 \pm 0{,}012$ (maligno). Esses valores indicam fidelidade apenas moderada e claramente inferior à de geradores de mamografia de alta qualidade, cujos FID situam-se tipicamente entre 20 e 60, muito abaixo dos aproximadamente 200--250 aqui observados. Essa limitação é coerente com o tamanho reduzido do conjunto de treinamento (439 imagens reais de DEV) e com a resolução de $128 \times 128$ pixels, e ajuda a explicar por que as imagens sintéticas não trouxeram ganho ao classificador.

\section{Classificação de mamografias}

Os resultados da avaliação perceptual sugerem que o modelo generativo reproduziu apenas de forma aproximada a distribuição visual das mamografias reais, com fidelidade agregada limitada (FID em torno de 200--250), coerente com o tamanho reduzido do conjunto e a baixa resolução. Ainda assim, as imagens produzidas preservam parte das características estruturais globais das mamografias reais, o que motiva investigar, de forma controlada, sua utilização como complemento no treinamento de modelos de classificação. Assim, procedeu-se à etapa de avaliação quantitativa, na qual se investigou o impacto da inclusão dessas imagens sintéticas no desempenho do classificador \textit{EfficientNet-B0}.
...
\textbf{Apenas reais (0:1)} & \textbf{0,614 [0,584; 0,647]} & \textbf{0,589 [0,552; 0,629]} & \textbf{0,663 [0,633; 0,698]} \\
...
\begin{quote}
\footnotesize \textit{Observação:} os valores da Tabela~\ref{tab:efficientnet_results} provêm da reexecução da \textit{pipeline} sob o protocolo livre de vazamento (\textit{hold-out} em nível de paciente, 10 sementes), substituindo integralmente os números da versão anterior, obtidos sob arranjo com vazamento de dados. A linha de base apenas com dados reais é a melhor configuração em todas as métricas; as proporções sintéticas não a superam e, em 1:1 e 2:1, tendem a piorar o F1-\textit{score}.
\end{quote}

A análise comparativa entre proporções, conduzida sobre as métricas regeneradas com os testes formais descritos na Seção~\ref{sec:analise_estatistica}, revela um resultado nulo. Em relação à linha de base composta apenas por dados reais, nenhuma proporção sintética apresentou ganho de AUC estatisticamente significativo pelo teste de Wilcoxon pareado com correção de Holm ($p_{\text{aj}} = 1{,}000$ em todas as proporções); a acurácia igualmente não diferiu de forma significativa ($p_{\text{aj}} = 1{,}000$), e o F1-\textit{score} tampouco atingiu significância em qualquer proporção. Os tamanhos de efeito foram nulos ou negativos, com delta de Cliff variando de $-0{,}18$ (proporção 1:1) a $+0{,}12$ (proporção 4:1), indicando que, quando há alguma diferença, ela tende a favorecer a linha de base. O teste de DeLong, aplicado às diferenças de AUC sobre as predições agregadas do conjunto de teste, também não foi significativo em nenhuma proporção ($p$ entre 0,124 e 0,513). Em conjunto, esses resultados indicam que a augmentação com imagens sintéticas \textbf{não melhora} o desempenho do classificador sob avaliação rigorosa: a configuração apenas com dados reais é a melhor em todas as métricas, e as proporções 1:1 e 2:1 chegam a apresentar leve tendência de piora. O efeito benéfico do dado sintético é, portanto, ausente neste cenário.

Esse achado contrasta diretamente com o ganho relatado na literatura e na versão anterior deste trabalho, o que motivou uma análise de ablação para investigar sua origem. Reexecutou-se a mesma \textit{pipeline} substituindo apenas a separação em nível de paciente por uma separação em \textbf{nível de imagem}, na qual imagens de uma mesma paciente podem ser distribuídas simultaneamente entre treino e teste (vazamento de dados). Sob esse protocolo mais fraco, o mesmo pipeline ``recupera'' um ganho de AUC de 0,611 para 0,683, estatisticamente significativo (teste de Wilcoxon pareado com correção de Holm, $p_{\text{aj}} = 0{,}047$). Como a única diferença entre os dois protocolos é o critério de separação, conclui-se que o ganho reportado sob o arranjo com vazamento é um \textbf{artefato metodológico} — decorrente do vazamento entre pacientes e do \textit{split} em nível de imagem — e não um efeito real das imagens sintéticas. Esse resultado explica por que ganhos aparentes desaparecem quando a avaliação é conduzida de forma rigorosa, em nível de paciente e sem vazamento.
...
\begin{table}[H]
\centering
\caption{Validação externa no MIAS (modelo treinado no CBIS-DDSM, sem retreinamento). Média [IC 95\%]. A configuração apenas com dados reais (em negrito) é a melhor; todas as proporções sintéticas ficam no nível do acaso e abaixo da linha de base, sem significância.}
\label{tab:validacao_externa}
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{0.95\textwidth}{l *{3}{>{\centering\arraybackslash}X}}
\toprule
\textbf{Proporção} & \textbf{Acurácia [IC 95\%]} & \textbf{F1} & \textbf{AUC [IC 95\%]} \\ \midrule
\textbf{Apenas reais (0:1)} & \textbf{0,542 [0,520; 0,563]} & \textbf{0,455} & \textbf{0,533 [0,512; 0,554]} \\
1:1 & 0,519 & 0,347 & 0,497 [0,479; 0,517] \\
2:1 & 0,514 & 0,394 & 0,503 [0,468; 0,538] \\
3:1 & 0,509 & 0,390 & 0,492 [0,474; 0,510] \\
4:1 & 0,522 & 0,325 & 0,500 [0,474; 0,525] \\
\bottomrule
\end{tabularx}
\end{table}

Os resultados indicam que o modelo treinado no CBIS-DDSM \textbf{não generaliza} para o MIAS e que, também fora da distribuição de origem, a augmentação sintética \textbf{não ajuda}. A linha de base apenas com dados reais é a melhor configuração, ainda que próxima ao acaso (AUC 0,533), enquanto todas as proporções sintéticas ficam entre 0,49 e 0,50 — no nível do acaso e \textbf{abaixo} da linha de base. Nenhuma proporção é significativa (teste de Wilcoxon com correção de Holm sobre a AUC e teste de DeLong, $p$ não significativos), e o delta de Cliff é nulo ou negativo, confirmando a ausência de benefício. A grande diferença de domínio — filme digitalizado (MIAS) contra mamografia digital de campo completo (CBIS-DDSM), com \textit{scanner} e população distintos, somada ao \textit{downsampling} agressivo de $1024$ para $128$ pixels — domina o resultado. Isso reforça o achado interno: a augmentação com dados sintéticos de fidelidade limitada não produz ganho nem dentro nem fora da distribuição de origem, corroborando o caráter exploratório de viabilidade do estudo, e não uma evidência de robustez clínica. Uma validação multi-institucional e multi-\textit{scanner} (por exemplo, INbreast e VinDr-Mammo) e o uso de maior resolução permanecem como trabalho futuro.
...
Trata-se de um estudo exploratório de viabilidade, cuja principal contribuição é um protocolo reprodutível e livre de vazamento de dados para mensurar o efeito do aumento com imagens sintéticas, e não uma afirmação de robustez clínica ou de melhoria garantida de desempenho. Os resultados quantitativos definitivos são obtidos sob o protocolo corrigido, com \textit{hold-out} em nível de paciente e múltiplas sementes, e a decisão sobre o efeito das imagens sintéticas baseia-se em testes de significância estatística (Wilcoxon pareado com correção de Holm, intervalos de confiança por \textit{bootstrap}, delta de Cliff e teste de DeLong para a AUC). Os números da versão anterior, obtidos sob arranjo com vazamento de dados, não são reaproveitados. Sob essa avaliação rigorosa — separação por paciente, ausência de vazamento, classificador funcional ajustado por \textit{fine-tuning} e fidelidade limitada do gerador (FID em torno de 200--250) — a augmentação com mamografias sintéticas geradas por GAN \textbf{não melhorou} a classificação: nenhuma proporção superou ou diferiu significativamente da linha de base composta apenas por dados reais ($p_{\text{aj}} = 1{,}000$ para AUC e acurácia; delta de Cliff nulo ou negativo; DeLong não significativo), sendo essa linha de base a melhor configuração, sem qualquer indício de memorização. Uma análise de ablação foi decisiva para interpretar o achado: ao substituir apenas a separação por paciente por uma separação em nível de imagem, que admite vazamento entre treino e teste, o mesmo pipeline ``recupera'' um ganho significativo (AUC de 0,611 para 0,683, $p_{\text{aj}} = 0{,}047$). Isso demonstra que os ganhos aparentes reportados na literatura e na versão anterior deste trabalho são \textbf{artefatos metodológicos} — de vazamento e \textit{split} em nível de imagem — e não efeito real dos dados sintéticos. A validação externa no MIAS, sem retreinamento, corroborou a ausência de benefício: o modelo \textbf{não generaliza} para uma base de domínio distinto (AUC próxima ao acaso) e a augmentação sintética também não ajuda fora da distribuição de origem. Não se afirma que a augmentação com dados sintéticos \emph{não possa} ajudar; mostra-se que, com um gerador de fidelidade apenas moderada e avaliação rigorosa em nível de paciente, os ganhos comumente reportados não se reproduzem, e um benefício real exigiria síntese de maior fidelidade e resolução, avaliada sem vazamento.
